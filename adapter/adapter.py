...
# Specific implementation of the adpater to read XML Source data
class XMLContactsAdapter(ContactsAdapter):
    def get_contacts(self):
    # Parse XML data into an ElementTree object
        root = ET.fromstring(self.data_source.read()) # Read XML file
        # Extract contact information from XML data
        contacts = []
        for elem in root.iter():
            if elem.tag == "contact":
                ...
                contacts.append(Contact(full_name, email, phone_number, is_friend))
        return contacts
